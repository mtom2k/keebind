...
- **Running a saved binding**: always call `executeBinding()` in `src/main/bindings/execution.ts`, never `runAction()` directly. That is the single confirmation, window-ownership, stale-record validation, duplicate-suppression and error-notification boundary used by global shortcuts and manual Run buttons. The custom prompt lifecycle lives in `src/main/confirmation.ts` and renders at `#confirmation`.
...
- **Working on auxiliary windows**: the tray popover, About panel and confirmation prompt load the same renderer bundle at `#popover`, `#about` and `#confirmation` (see `src/renderer/src/main.tsx`). In a browser you can reach them by editing the hash and reloading, which is how they get styled and checked without Electron. The popover sizes its own window by reporting `offsetHeight` over `popover:resize`, so wrap its content in the element the `useLayoutEffect` measures.
...
- **Regenerating README screenshots**: `npm run screenshots`. It builds, then loads `out/renderer/index.html` in a bare Electron window, where the absent preload bridge makes `devMock.ts` install itself, so the shots are the real UI on sample data with no permissions or hardware involved. Sample bindings, the fake "packaged" permission state and each interaction live in `scripts/capture-screenshots.mjs`; `docs/screenshots/*.png` are outputs, never hand-edited. Re-run it after any visual change. Two traps: the script must keep its own `window-all-closed` no-op handler, because destroying one shot's window would otherwise quit Electron before the next; and each `executeJavaScript` call shares one global scope, so injected snippets are wrapped in a function scope (`run()`) rather than declaring `const` at top level.
...
- **Popover confirmation replaces the popover.** A guarded binding launched from the tray hides the tray panel and opens the dedicated confirmation surface. A global-hotkey prompt is standalone and must never reveal the hidden main window. Only a prompt initiated from an already-visible main window is parented to that window.
- **Create a fresh popover for every presentation.** Do not change `hidePopover()` back to retaining a hidden transparent `BrowserWindow`: Windows re-composites that retained window with a visible appear/disappear/reappear flash on later tray clicks, while first creation is stable. Dismissal therefore destroys the window. The next `showPopover()` creates a fresh renderer, leaves it hidden until `resizePopover()` receives the loaded content height, and shows it once. `scheduleShow()` defers presentation until the notification-area click completes, while the blur handler coalesces transient focus handoffs. The `popover:hide` IPC handler defers destruction until after replying to its renderer.
