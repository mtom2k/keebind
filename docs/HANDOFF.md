...
- **UI work without Electron**: `npm run dev`, then open http://localhost:5173 in a normal browser. `devMock.ts` auto-installs a fake `window.keebind` (in-memory bindings, a fake 3×3 VIA macropad, key events from the browser). This is how most UI iteration happens, with no hardware and no permissions.
...
- **node-hid writes need a leading `0x00` report-id byte** on every platform; VIA responses echo the command id in byte 0, so `ViaClient.request` loops reads until it sees the echo.
...
- **Input Monitoring can't be queried**, only requested. Two calls are gated on it: opening a keyboard-class HID device, and creating a keyboard event tap. `requestInputMonitoring()` tries both, because a Mac with no enumerable HID keyboard would otherwise never appear in the pane. Both prompt, so they run on explicit action only and the answer is cached.
