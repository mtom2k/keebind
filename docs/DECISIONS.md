...
## 33. Dedicated confirmation window and stable tray presentation (2026-08-01)
...
**Decision:** Render guarded-run confirmations in a dedicated frameless KeeBind window using the shared renderer bundle at `#confirmation`. Only an already-visible main window owns the prompt; global prompts are standalone and never reveal the main application, while tray-launched prompts replace the popover. Restrict confirmation data and responses to the active confirmation renderer. For the tray, keep every opening hidden until that opening's refreshed binding list has produced a new content-height report, defer presentation until the click callback completes, resize visible content without calling `show()` again, and delay blur dismissal briefly so Windows' transient blur/focus pair is treated as one focus handoff rather than a real outside click.

**Consequences:** Confirmations now match KeeBind's light/dark design on macOS and Windows, retain deny-by-default behavior, and disclose the same full binding details without flashing the main application. The tray popover appears once at its freshly measured size on both its first and subsequent openings, and does not disappear during the notification area's focus transfer; a genuine outside click dismisses it after a 150 ms delay. The custom confirmation adds one renderer route and two narrowly scoped IPC operations that must remain synchronized with the preload API and browser mock.

## 34. Dispose the transparent tray window after every use (2026-08-01)

**Context:** The tray popover's first opening was consistently stable, but every later opening visibly appeared, disappeared and reappeared despite waiting for fresh content measurement and coalescing transient blur/focus events. The remaining difference was native window identity: the first click created a new transparent `BrowserWindow`, while later clicks re-showed the same window after `hide()`. The renderer content and geometry were already ready before presentation, isolating the flash to Windows re-compositing the retained transparent native window.
