...
| Hotkey → action bindings | ✅ Done | Electron `globalShortcut`. Actions: launch app, open URL, open path, shell command, multi-step workflow with per-step delays. Browse buttons open Finder / File Explorer for app and path targets, in single actions and workflow steps. Dynamic search filters by name, hotkey, action and target. Deleting requires confirmation. Per-binding enable toggle, Run button, master switch. "Not registered" badge when the OS refuses a hotkey. |
...
- Bindings search: typing filters immediately across descriptions, accelerators, action summaries, workflow steps and targets; result count, clear action and no-match state update with it.
