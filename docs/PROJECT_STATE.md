...
| Key Listener | ✅ Done | `uiohook-napi` global hook, opt-in start/stop. Renders whole chords as keycaps (modifiers included) or opens a new binding with that accelerator prefilled. Accelerator strings, numeric keycodes and Copy accelerator are hidden by default behind the Show technical key details setting. Clear button and history table included. |
